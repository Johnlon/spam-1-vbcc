...
static void clear_main_ret(void) {
	if (c99
			&& !strcmp(cur_func,
					"main")&&return_typ&&ISSCALAR(return_typ->flags)) {
...
{
	if (DEBUG & 2)
		printf("settyp: new=%d old=%d\n", typnew, typold);
	if (typold == LONG && typnew == FLOAT) {
		error(203);
		return DOUBLE;
	}
	if (typold == LONG && typnew == DOUBLE)
		return LDOUBLE;
	if (c99 && typold == LONG && typnew == LONG)
		return LLONG;
	if (typold == INT && (typnew == SHORT || typnew == LONG))
		return typnew;
	if (typold != 0 && typnew != INT) {
		error(47);
		return (typnew);
	}
	if (typold == 0 && typnew == INT)
	...
	if (typold == 0)
		return (typnew);
	if (typold == SHORT || typold == LONG || typold == LLONG)
		return (typold);
	error(48);
	return (typnew);
}
...
/* parst z.B. unsigned int, bla etc.               */
{
	...
	do {
		...
		notdone = 0;
		if (ctok->type == NAME) {
			if (!strcmp("struct", ctok->name) || !strcmp("union", ctok->name)
					|| (ecpp && !strcmp("class", ctok->name))) {
				if (!strcmp("struct", ctok->name))
					notdone = STRUCT;
...
/* removed */
#endif
					if (ssd && ctok->type == LBRA
							&& find_struct(sident, nesting) && ssd->count > 0)
						error(13, sident);
					if (!ssd
							|| ((ctok->type == LBRA || ctok->type == SEMIC)
									&& !find_struct(sident, nesting))) {
...
/* removed */
							...
							if (type_expression(tree, 0)) {
								...
								tsize = (int) zm2l(
										zmmult(sizetab[t->flags&NQ],char_bit));
								if (bfsize < 0 || bfsize > tsize
										|| (bfsize == 0 && *ident)) {
									error(332);
									bfsize = 1;
								}
...
/* removed */
						...
						if (type_uncomplete(t)) {
							if (!c99 || notdone != STRUCT || flex_array
									|| (t->flags & NQ) != ARRAY
									|| type_uncomplete(t->next)) {
								error(14, sbuff);
								freetyp(t);
								break;
							} else {
...
/* removed */
						...
						(*sl)[scount].styp = t;
						if (!ecpp)
							(*sl)[scount].identifier = add_identifier(ident,
									strlen(ident));
#ifdef HAVE_ECPP
						/* removed */
						/* removed */
							...
							(*sl)[scount].align = falign(t);
						{
							...
							zmax sz = szof(t);
							m = off;
							if ((zmeqto(al,l2zm(0L)) || zmeqto(sz, l2zm(0L)))
									&& !flex_array) {
								if (!ecpp) {
									error(316, ident);
								}
								...
								off = zmmult(
										zmdiv(zmadd(off,zmsub(al,l2zm(1L))),al),
										al);
								if (!zmeqto(m, off))
									error(306, ident);
								if (!flex_array) {
									m =
											zmmult(
													zmdiv(zmadd(off,zmsub(sz,l2zm(1L))),sz),
													sz);
									if (!zmeqto(m, off))
										error(307, ident);
									off = zmadd(off, sz);
...
/* removed */
				...
				if (ctok->type == LBRA) {
					...
					while (ctok->type != RBRA) {
						...
						if (ctok->type != RBRA && ctok->type != COMMA) {
							error(56);
							break;
						}
						if (ctok->type == COMMA)
				...
				killsp();
				typ = settyp(INT, typ);
			} else if (!strcmp("__readsmem", ctok->name)
					|| !strcmp("__writesmem", ctok->name)) {
				enum {
					READS, WRITES
				} op;
				...
				if (tmp) {
					...
					sprintf(vattr + strlen(vattr), ";%s(%d)",
							op == READS ? "readmem" : "writemem", tmp->flags);
				}
				killsp();
				if (ctok->type == RPAR)
					next_token();
				...
				/* removed */
#endif
			} else if (/*!(c_flags[7]&USEDFLAG)&&*/!strcmp("__reg",
					ctok->name)) {
				char *d;
				next_token();
				killsp();
				...
				killsp();
				notdone = 1;
			} else if (/*!(c_flags[7]&USEDFLAG)&&*/!strcmp("__attr",
					ctok->name)) {
				char *d;
				next_token();
				killsp();
				...
				if (ctok->type == RPAR)
					...
					error(59);
				killsp();
			} else if (/*!(c_flags[7]&USEDFLAG)&&*/!strcmp("__vattr",
					ctok->name)) {
				char *d;
				next_token();
				killsp();
...
#endif
				if (!notdone && typ == 0
						&& !(type_qualifiers & (XSIGNED | UNSIGNED))) {
	...
	while (ctok->type == STAR) {
		...
		do {
			...
			if (ctok->type == NAME && !strcmp("const", ctok->name)) {
				...
				notdone = 1;
				next_token();
			} else if (c99 && ctok->type == NAME
					&& !strcmp("restrict", ctok->name)) {
				a->flags |= RESTRICT;
				notdone = 1;
				next_token();
...
/* removed */
				...
				(*sl)[fsd->count].storage_class = return_sc;
				{
					int m = nesting;
					nesting = 0;
					(*sl)[fsd->count].identifier = add_identifier(ident,
							strlen(ident));
					nesting = m;
				}
				if (t) {
					if (((*sl)[fsd->count].styp->flags & NQ) == VOID
							&& fsd->count != 0)
						error(22);
					/*  Arrays in Zeiger umwandeln  */
					if (ISARRAY((*sl)[fsd->count].styp->flags))
						(*sl)[fsd->count].styp->flags = POINTER_TYPE(
								(*sl)[fsd->count].styp->next);
				...
				if (hard_reg && regok(hard_reg, t->flags, -1) <= 0)
					error(217, regnames[hard_reg]);
#if 0 /*#ifdef HAVE_REGPARMS*/
...
/* removed */
...
#ifdef HAVE_REGPARMS
			{
				...
				for (i = 0; i < p->exact->count; i++) {
					if ((*p->exact->sl)[i].styp)
						r = reg_parm(&reg_handle, (*p->exact->sl)[i].styp, 0,
								p);
					else
						r = 0;
					if ((*p->exact->sl)[i].reg == 0)
						(*p->exact->sl)[i].reg = r;
				}
			}
#endif
			...
			if (ctok->type != RPAR)
				...
				next_token();
			killsp();
			if (ctok->type == COMMA || ctok->type == SEMIC || ctok->type == RPAR
					|| ctok->type == ASGN)
				leave_block();
			if (last) {
				last->next = p;
...
/*  im Source.                                              */
{
	...
	if (ctok->type == NAME) {
		if (!strcmp("auto", ctok->name))
			...
			fl = 1;
		else if (!strcmp("struct", ctok->name))
			fl = 1;
		else if (!strcmp("typedef", ctok->name))
			fl = 1;
		else if (!strcmp("union", ctok->name))
			fl = 1;
		else if (!strcmp("unsigned", ctok->name))
			fl = 1;
		else if (!strcmp("void", ctok->name))
			...
			fl = 1;
		else if (/*!(c_flags[7]&USEDFLAG)&&*/!strcmp("__mask", ctok->name))
			fl = 1;
		else if (/*!(c_flags[7]&USEDFLAG)&&*/!strcmp("__readsmem", ctok->name))
			fl = 1;
		else if (/*!(c_flags[7]&USEDFLAG)&&*/!strcmp("__writesmem", ctok->name))
			fl = 1;
		else {
			v = find_var(ctok->name, 0);
			if (v && v->storage_class == TYPEDEF)
...
{
	identifier_list *new;
	if ((*identifier == 0 && length == 0) || identifier == empty)
		return (empty);
...
/*  unvollstaendig, aber keine Zeiger oder Funktionen darauf.   */
{
	...
	if (ISSTRUCT(p->flags) || ISUNION(p->flags))
		if (p->exact->count <= 0)
			return 1;
	if (ISARRAY(p->flags)) {
		if (!p->dsize && zmleq(p->size, l2zm(0L)))
			return 1;
		if (!p->next)
			ierror(0);
		if (type_uncomplete(p->next))
			return 1;
	}
	return 0;
}
...
/*  vervollstaendigt unvollstaendige Typen dieser Struktur.     */
{
	...
	if (first_si[nesting] == 0) {
		first_si[nesting] = new;
		last_si[nesting] = new;
	} else {
		last_si[nesting]->next = new;
		last_si[nesting] = new;
	}
	...
	for (i = nesting; i >= endnesting; i--) {
		...
		while (si) {
			if (!strcmp(si->identifier, identifier)) {
				if (DEBUG & 1)
					printf("found tag <%s> at nesting %d->%p\n", identifier, i,
							(void*) si->sd);
				return (si->sd);
			}
#ifdef HAVE_MISRA
...
void vla_nesting(IC *p, Var **vn, int *nest) {
	if (p->code == CALL && (p->q1.flags & (VAR | DREFOBJ)) == VAR
			&& !strcmp(p->q1.v->identifier, "__oldvlasp")) {
		IC *p2 = p->next;
		while (p2 && (p2->code == ALLOCREG || p2->code == FREEREG))
			p2 = p2->next;
		if (!p2 || p2->code != GETRETURN
				|| (p2->z.flags & (VAR | DREFOBJ)) != VAR)
			ierror(0);
	...
	if (p->code == CALL && (p->q1.flags & (VAR | DREFOBJ)) == VAR
			&& !strcmp(p->q1.v->identifier, "__resetvlasp")) {
		/*printf("found reset\n");*/
		if (*nest <= 0)
			ierror(0);
		(*nest)--;
...
void vla_jump_fix(void) {
	...
	if (DEBUG & 1)
		printf("searching for vla-jump-fixes\n");
	for (p = first_ic; p; p = p->next) {
		/*pric2(stdout,p);*/
		if (p->code >= BEQ && p->code <= BRA) {
			...
			if (1/*nest>0*/) {
				...
				if (return_vla_nest > nest
						|| (nest > 0 && return_vla_nest == nest
								&& savedsp != vn[nest - 1])) {
					err_ic = p;
					error(351);
					...
					if (vn[nest]) {
						...
						for (p2 = p->prev; p2; p2 = p2->prev) {
							if (p2->code == CALL
									&& (p2->q1.flags & (VAR | DREFOBJ)) == VAR
									&& !strcmp(p2->q1.v->identifier,
											"__oldvlasp")) {
								if (ndiff == 0) {
									/*printf("found savesp\n");*/
									p2 = p2->next;
...
/*  OLDSTYLE und REGPARM erkannt.                               */
{
	...
	/* removed */
#endif
	if (ISFUNC(t->flags&NQ)
			&& (ISARRAY(t->next->flags) || ISFUNC(t->next->flags)))
		error(25);
	...
	if (DEBUG & 2)
		printf("storage_class=%d\n", storage_class);
	if (storage_class & PARAMETER)
		new->flags |= USEDASDEST;
	...
	if (is_vlength(t) && storage_class != AUTO && storage_class != REGISTER)
		error(315);
	if ((storage_class & PLAIN_STORAGE_CLASS) == REGISTER)
		new->priority = registerpri;
	else
		new->priority = 0;
	if (/*nesting==0&&*/new->storage_class == EXTERN) {
		...
		if (hash_ext)
			add_hashentry(hash_ext, new->identifier, new);
	} else {
		new->identifier = add_identifier(identifier, strlen(identifier));
		if (last_var[nesting]) {
			new->offset = zmadd(last_var[nesting]->offset,
					szof(last_var[nesting]->vtyp));
	...
	f = t->flags & NQ;
	if ((storage_class & PLAIN_STORAGE_CLASS) == AUTO
			|| (storage_class & PLAIN_STORAGE_CLASS) == REGISTER) {
		if (DEBUG & 2)
			printf("auto\n");
		al = falign(t);
		if ((c_flags_val[0].l & 2) && nesting == 1
				&& !(storage_class & PARAMETER)) {
			...
			if (storage_class & PARAMETER) {
				...
				if (align_arguments)
					new->offset = zmmult(
							zmdiv(zmadd(new->offset,zmsub(al,l2zm(1L))),al),
							al);
			} else {
				new->offset = local_offset[nesting];
				new->offset = zmmult(
						zmdiv(zmadd(new->offset,zmsub(al,l2zm(1L))),al), al);
			}
		}
		if (storage_class & PARAMETER) {
			new->offset =
					zmmult(
							zmdiv(zmadd(new->offset,zmsub(stackalign,l2zm(1L))),stackalign),
							stackalign);
			if (ISINT(f) && f < INT && !short_push) {
				...
				if (f == FLOAT && (storage_class & OLDSTYLE)) {
					...
					new->offset =
							zmmult(
									zmdiv(zmadd(new->offset,zmsub(align[DOUBLE],l2zm(1L))),align[DOUBLE]),
									align[DOUBLE]);
		...
		if (!(storage_class & PARAMETER))
			if (zmleq(max_offset, local_offset[nesting]))
				max_offset = local_offset[nesting];
		if (DEBUG & 2)
			printf("max_offset=%ld\n", zm2l(max_offset));
	}
	if ((storage_class & PLAIN_STORAGE_CLASS) == STATIC)
		new->offset = l2zm((long )++label);
	if (storage_class & PARAMETER) {
		...
		if (ISINT(f) && f < INT && !zmleq(sizetab[INT], sizetab[f])) {
			if (BIGENDIAN) {
				new->offset = zmadd(new->offset,
						zmsub(sizetab[INT],sizetab[f]));
			} else {
				if (!LITTLEENDIAN)
					ierror(0);
			}
	...
	if ((storage_class & PLAIN_STORAGE_CLASS) == EXTERN) {
		if (!strcmp("fprintf", identifier))
			new->flags |= PRINTFLIKE;
		if (!strcmp("printf", identifier))
			new->flags |= PRINTFLIKE;
		if (!strcmp("sprintf", identifier))
			new->flags |= PRINTFLIKE;
		if (!strcmp("fscanf", identifier))
			new->flags |= SCANFLIKE;
		if (!strcmp("scanf", identifier))
			new->flags |= SCANFLIKE;
		if (!strcmp("sscanf", identifier))
			new->flags |= SCANFLIKE;
	}
...
/*  Gibt Variablenliste inkl. Typ, aber ohne Identifier frei.   */
{
	Var *merk;
	while (p) {
		if (!*p->identifier && p->identifier != empty)
			ierror(0);
		free(p->description);
		free(p->vattr);
		merk = p->next;
		if (!(p->flags & USEDASADR)
				&& (p->storage_class == AUTO || p->storage_class == REGISTER)) {
			if (*p->identifier && !(p->flags & USEDASDEST)
					&& ISSCALAR(p->vtyp->flags))
				error(64, p->identifier);
			if (*p->identifier && !(p->flags & USEDASSOURCE)
					&& ISSCALAR(p->vtyp->flags))
				error(65, p->identifier);
		}
		if (DEBUG & 2)
			printf("free_var %s, pri=%d\n", p->identifier, p->priority);
		if (p->vtyp)
			freetyp(p->vtyp);
		if (p->clist)
			free_clist(p->clist);
		if (p->fi) {
			if (DEBUG & 2)
				printf("free_fi of function %s\n", p->identifier);
			free_fi(p->fi);
			if (DEBUG & 2)
				printf("end free_fi of function %s\n", p->identifier);
		}
		free(p);
		p = merk;
	}
...
Var* find_ext_var(char *identifier) {
	...
	if (misracheck)
		l = strlen(identifier);
	if (hash_ext)
		return find_name(hash_ext, identifier);
...
int use_only_dyn_init(zmax sz, zmax dyn_sz, zmax const_sz, int dyn_cnt,
		int const_cnt) {
...
void init_local_compound(Var *v) {
	if (v->storage_class == AUTO || v->storage_class == REGISTER) {
		...
		if (v->clist->tree) {
			...
			/*  Array etc.  */
			Var *nv;
			if (!ISSCALAR(v->vtyp->flags)
					&& !use_only_dyn_init(szof(v->vtyp), init_dyn_sz,
							init_const_sz, init_dyn_cnt, init_const_cnt)) {
...
/*  noetigen Strukturen.                                        */
{
	type *ts, *t, *old = 0, *om = 0;
	char *imerk, vident[MAXI];
	...
	if (for_decl
			&& storage_class
					!= 0&&storage_class!=AUTO&&storage_class!=REGISTER) {
		error(299);
		storage_class = AUTO;
	}
	...
	if (ctok->type == SEMIC) {
		if (!ts)
			error(0);
		if (storage_class
				|| (!ISSTRUCT(ts->flags) && !ISUNION(ts->flags)
						&& (ts->flags & NQ) != INT))
			error(36);
		freetyp(ts);
		next_token();
...
/* removed */
	...
	while (notdone) {
		...
		imerk = ident;
		ident = vident;
		*vident = 0; /* merken von ident hier vermutlich */
		storage_class = msc;
		hard_reg = mhr;
		if (old) {
			freetyp(old);
		...
		if (!ISFUNC(t->flags)) {
			...
			/* removed */
#endif
			if (storage_class != STATIC && storage_class != TYPEDEF)
				storage_class = EXTERN;
		}
		...
		if (!v && /*oldnesting==0&&*/storage_class == EXTERN && cross_module
				&& (v = find_ext_var(vident))) {
			v->flags &= ~NOTINTU;
			diffunit = 1;
		...
		if (v) {
			...
			if (storage_class == TYPEDEF) {
				...
				if (nesting > 0 && (v->flags & DEFINED) && !extern_flag
						&& !isfunc) {
					error(27, vident);
				} else {
					if (t && v->vtyp
							&& !compatible_types(v->vtyp, t,
									NU | CONST | VOLATILE)) {
...
/* removed */
...
#ifdef HAVE_TARGET_ATTRIBUTES
				{
					int i;
					for (i = 0; g_attr_name[i]; i++) {
						if ((v->tattr & (1L << i)) != (tattr & (1L << i)))
							error(228, vident, g_attr_name[i]);
					}
					v->tattr = tattr;
				}
#endif
				if (vattr) {
					if (!v->vattr || !strstr(v->vattr, vattr)) {
						error(370, v->identifier, vattr,
								v->vattr ? v->vattr : empty);
			...
			/* removed */
#endif
			if (isfunc && ctok->type != COMMA && ctok->type != SEMIC
					&& ctok->type != RPAR && ctok->type != ASGN && nesting > 0)
				nesting--;
		...
		/* removed */
#endif
		if (disallow_statics && v->storage_class == STATIC && *v->identifier
				&& !is_const(v->vtyp))
			error(302, v->identifier);
		killsp();
		/*  Inline-Assembler-Code in Funktionsdeklarationen */
		if (ctok->type == ASGN && ISFUNC(v->vtyp->flags)
				&& (header_cnt > 0 || !(c_flags[7] & USEDFLAG))) {
			np tree;
			next_token();
			killsp();
			if (v->fi) {
				free(v->fi->inline_asm);
				v->fi->inline_asm = 0;
			}
			if (!v->fi)
				v->fi = new_fi();
		...
		if (ctok->type == ASGN) {
			...
			if (!had_decl && v->nesting == 0 && v->storage_class == EXTERN
					&& strcmp("main", v->identifier))
				error(168, v->identifier);
			if (v->flags & DEFINED) {
				if (nesting == 0)
					error(30, v->identifier);
			} else {
				v->flags |= DEFINED;
				v->dfilename = filename;
				v->dline = line;
...
/* removed */
			...
			if (v->storage_class == TYPEDEF)
				error(114, v->identifier);
			if (extern_flag) {
				if (nesting == 0)
...
/* removed */
			...
			init_const_cnt = 0;
			v->clist = initialization(v->vtyp,
					v->storage_class == AUTO || v->storage_class == REGISTER, 0,
					0, 0, 0);
			...
			if (v->clist) {
				if (ISARRAY(v->vtyp->flags) && zmeqto(v->vtyp->size, l2zm(0L))) {
					...
					while (p) {
						v->vtyp->size = zmadd(p->idx, l2zm(1L));
						p = p->next;
					}
					if (v->storage_class == AUTO || v->storage_class == REGISTER) {
						local_offset[nesting] = zmadd(local_offset[nesting],
								szof(v->vtyp));
						if (zmleq(max_offset, local_offset[nesting]))
							max_offset = local_offset[nesting];
					}
			...
			if ((v->flags & DEFINED) && type_uncomplete(v->vtyp))
				error(202, v->identifier);
			if ((v->vtyp->flags & CONST)
					&& (v->storage_class == AUTO || v->storage_class == REGISTER))
...
#endif
		if (DEBUG & 1)
			printf("Funktionsdefinition! %s %p\n", v->identifier, (void*) v);
		{
			int i;
			for (i = 1; i <= MAXR; i++) {
				regs[i] = regused[i] = regsa[i];
				regsbuf[i] = 0;
			}
		}
...
/* removed */
		...
		if (storage_class != EXTERN && storage_class != STATIC)
			error(34);
		if (extern_flag && !inline_flag)
			error(120);
		if (storage_class == EXTERN && !strcmp(v->identifier, "main")
				&& (!t->next || t->next->flags != INT))
			error(121);
		if (!had_decl && v->nesting == 0 && v->storage_class == EXTERN
				&& strcmp("main", v->identifier))
			error(168, v->identifier);
		while (!ecpp && ctok->type != LBRA) {
			/*  alter Stil  */
			type *nt = declaration_specifiers();
			notdone = 1;
			oldstyle = OLDSTYLE;
			if (!ts) {
				error(35);
			...
			if (ctok->type == SEMIC) {
				...
				error(54);
				while (ctok->type != LBRA && ctok->type != SEMIC) {
					next_token();
					killsp();
				}
			}
		...
		if (v->vtyp->next->flags == VOID) {
			...
			if (!ffreturn(return_typ) && (return_typ->flags & NQ) != VOID) {
				...
				type *rt = new_typ();
				int reg;
				rt->flags = POINTER_TYPE(return_typ);
				rt->next = return_typ;
#ifdef HAVE_REGPARMS
				reg = reg_parm(&reg_handle, rt, 0, v->vtyp);
				if (!reg) {
					return_var = add_var(empty, clone_typ(rt),
							AUTO | PARAMETER | oldstyle, 0);
				} else {
					return_var = add_var(empty, clone_typ(rt),
							reg < 0 ?
									(AUTO | PARAMETER | REGPARM | DBLPUSH
											| oldstyle) :
									(AUTO | PARAMETER | REGPARM | oldstyle), 0);
					return_var->reg = reg;
				}
#else
		...
		ic_count = 0;
		max_offset = l2zm(0L);
		if (!zmleq(local_offset[1], Z0))
			max_offset = local_offset[1];
		for (i = 0; i < t->exact->count; i++) {
...
#endif
				if ((*t->exact->sl)[i].reg > 0)
					sc |= REGPARM;
				if ((*t->exact->sl)[i].reg < 0)
					sc |= (REGPARM | DBLPUSH);
				tmp = add_var((*t->exact->sl)[i].identifier,
						clone_typ((*t->exact->sl)[i].styp), sc, 0);
...
/* removed */
		...
		if (oldstyle)
			t->exact->count = 0; /*  Prototype entfernen */
		/*        local_offset[1]=l2zm(0L);*/
		return_label = ++label;
		function_calls = 0;
		float_used = 0;
		...
		if (c99) {
			...
			fnc = add_var("__func__", ft, STATIC,
					cl_from_string(cur_func, cur_func + strlen(cur_func)));
			fnc->flags |= DEFINED;
		}
		...
		if ((v->vtyp->next->flags & NQ) != VOID && !has_return) {
			if (strcmp(v->identifier, "main"))
				error(173, v->identifier);
			else
				error(174, v->identifier);
		}
#if 0
		{int i;
					...
					for (i = 1; i <= MAXR; i++) {
						if (reg_pair(i, &rp)) {
							if (BTST(regs_modified, i)) {
								...
								if (BTST(regs_modified,
										rp.r1)||BTST(regs_modified,rp.r2))
		...
		if (v->fi && v->fi->first_ic) {
			Var *vp;
			if (DEBUG & 1)
				printf("leave block %d (inline-version)\n", nesting);
			if (block_vla[nesting])
				clearvl();
			if (nesting != 1)
				ierror(0);
			if (merk_varl)
				merk_varl->next = first_var[nesting];
			else
				merk_varf = first_var[nesting];
			if (last_var[nesting])
				merk_varl = last_var[nesting];
			if (merk_sil)
				merk_sil->next = first_si[nesting];
			else
				merk_sif = first_si[nesting];
			if (last_si[nesting])
				merk_sil = last_si[nesting];
			if (merk_sdl)
				merk_sdl->next = first_sd[nesting];
			else
				merk_sdf = first_sd[nesting];
			if (last_sd[nesting])
				merk_sdl = last_sd[nesting];
			if (merk_ilistl)
				merk_ilistl->next = first_ilist[nesting];
			else
				merk_ilistf = first_ilist[nesting];
			if (last_ilist[nesting])
				merk_ilistl = last_ilist[nesting];

			if (merk_varf && !only_inline && !cross_module)
				gen_vars(merk_varf);
			if (first_llist)
				free_llist(first_llist);
			first_llist = 0;
			if (first_clist)
				free_clist(first_clist);
			first_clist = 0;
			if (merk_sif)
				free_si(merk_sif);
			...
			if (!cross_module) {
				vp = merk_varf;
				while (vp) {
					if (vp->storage_class == AUTO
							|| vp->storage_class == REGISTER) {
						/*if(DEBUG&1024) printf("handling variable %s(%ld)/%p\n",vp->identifier,zm2l(vp->offset),(void*)vp);*/
						if (!zmleq(l2zm(0L), vp->offset)) {
							vp->offset = l2zm(0L);
							if (DEBUG & 1024)
								printf(
										"converted parameter <%s>(%ld) for inlining\n",
										vp->identifier,
										(long) zm2l(vp->offset));
						} else
							vp->offset = l2zm(4L); /*  Dummy, da recalc_offsets?   */
					}
					vp = vp->next;
				}
			}
...
/*  werden laut ANSI nicht beruecksichtigt.                         */
{
	...
	bf &= NQ;
	if (ISFUNC(af)) {
		if (a->exact->count && !compare_sd(a->exact, b->exact))
			return (0);
	}
	if (ISSTRUCT(af) || ISUNION(af)) {
		if (cross_module && a->exact->tunit != b->exact->tunit)
			return 1;
		if (a->exact != b->exact)
			return (0);
	}
	if (ISARRAY(af)) {
		if (!zmeqto(a->size,
				l2zm(0L))&&!zmeqto(b->size,l2zm(0L))&&!zmeqto(a->size,b->size))
			return (0);
	}
...
/*  Wird nur nur fuer Prototypen benutzt, leere Liste immer gleich. */
{
	...
	if (a->count != b->count)
		return (0);
	for (i = 0; i < a->count; i++) {
		if ((*a->sl)[i].styp && (*b->sl)[i].styp
				&& !compatible_types((*a->sl)[i].styp, (*b->sl)[i].styp, NU))
			return (0);
		if ((*a->sl)[i].reg != (*b->sl)[i].reg) {
			error(368);
			return 0;
		}
#ifdef HAVE_MISRA
...
/*  Generiert Variablen.                                    */
{
	int mode, al, first_pass = 1;
	Var *p;
	if (errors != 0 || (c_flags[5] & USEDFLAG))
		return;
	if (optsize)
		al = zm2l(maxalign);
	else
		...
		for (mode = 0; mode < 3; mode++) {
			...
			for (p = v; p; p = p->next) {
				...
				if (optsize && zm2l(falign(p->vtyp)) != al)
					continue;
				if (cross_module && !(p->flags & REFERENCED))
					continue;
				if (DEBUG & 2)
					printf("gen_var(): %s\n", p->identifier);
				if (p->storage_class == STATIC || p->storage_class == EXTERN) {
					if (!(p->flags & GENERATED)) {
						if (p->storage_class == EXTERN
								&& !(p->flags & (USEDASSOURCE | USEDASDEST))
								&& !(p->flags & (TENTATIVE | DEFINED)))
							continue;
						if (p->storage_class == STATIC && p->nesting > 0
								&& !(p->flags & (USEDASSOURCE | USEDASDEST)))
							continue;
						...
						if (!(p->flags & (TENTATIVE | DEFINED))) {
							if (!((p->vtyp->flags & NQ) == FUNKT) || !p->fi
									|| !p->fi->inline_asm) {
								if (mask_opt) {
									if ((p->flags & PRINTFLIKE)
											&& !strstr(p->identifier, "."))
										needs("vfprintf");
									if ((p->flags & SCANFLIKE)
											&& !strstr(p->identifier, "."))
										needs("vfscanf");
								}
								gen_var_head(out, p);
							}
							if (p->storage_class == STATIC
									&& (!p->fi || !p->fi->inline_asm))
								error(127, p->identifier);
							continue;
						} else {
						...
						if (!p->clist) {
							if (type_uncomplete(p->vtyp))
								error(202, p->identifier);
...
void dynamic_init(Var *v, type *t, const_list *cl, zmax of, int noconst) {
	...
	if (ISARRAY(f)) {
		...
		dynamic_init(v, (*t->exact->sl)[cl ? zm2l(cl->idx) : 0].styp,
				cl ? cl->other : 0, of, noconst);
		...
		for (i = 0; i < t->exact->count && cl; i++) {
			...
			if (bfo >= 0) {
				...
				if (!zmeqto(cl->idx, l2zm(i)) || !cl->other) {
					...
					insert_bitfield(&dest, &cl->other->tree->o,
							&cl->other->tree->o, bfs, bfo, st->flags, 1);
					cl = cl->next;
				} else {
					static obj val = { 0 };
					val.flags = KONST;
				...
				if (i + 1 >= t->exact->count
						|| (*t->exact->sl)[i + 1].bfoffset <= 0 || !cl) {
					of = zmadd(of, szof(st));
				}
			} else {
...
/*  Generiert dc fuer const_list.                           */
{
	...
	if (ISARRAY(t->flags)) {
		...
		if (!zmleq(t->size, sz))
			gen_ds(f, zmmult(zmsub(t->size,sz), szof(t->next)), t->next);
		return;
	}
	if (ISUNION(t->flags)) {
		int i = zm2l(cl->idx);
	...
	if (ISSTRUCT(t->flags)) {
		...
		if (cl && cl->tree) {
			...
			for (i = 0; i < t->exact->count && cl; i++) {
				...
				if (bfo >= 0) {
					...
					if ((*t->exact->sl)[i].identifier[0]) {
						...
						vumax =
								zumand(vumax,
										zumsub(zumlshift(ul2zum(1UL),ul2zum((unsigned long)bfs)),ul2zum(1UL)));
						bfval =
								zumor(bfval,
										zumlshift(vumax,ul2zum((unsigned long)bflayout(bfo,bfs,st->flags))));
					}
					if (i + 1 >= t->exact->count
							|| (*t->exact->sl)[i + 1].bfoffset <= 0 || !cl) {
			...
			for (; i < t->exact->count; i++) {
				...
				gen_ds(f, szof((*t->exact->sl)[i].styp),
						(*t->exact->sl)[i].styp);
				sz = zmadd(sz, szof(st));
			}
		}
	...
	if (ISARRAY(f) && ctok->type == LBRK) {
		...
		if (!type_expression(tree, 0)) {
			...
			if (tree->flags != CEXPR || !ISINT(tree->ntyp->flags)) {
				...
				eval_constn(tree);
				if (!zmleq(l2zm(0L), vmax)) {
					error(354);
					vmax = l2zm(0L);
				}
				if (!zmeqto(t->size, l2zm(0L)) && zmleq(t->size, vmax)) {
					error(354);
					vmax = l2zm(0L);
				}
		...
		if (ctok->type != NAME) {
			...
			int i, n = -1;
			for (i = 0; i < t->exact->count; i++)
				if (!strcmp((*t->exact->sl)[i].identifier, ctok->name))
					n = i;
			if (n < 0) {
				error(23, ctok->name);
				return 0;
...
const_list* initialization(type *t, int noconst, int level, int desi,
		struct_declaration *fstruct, const_list *first)
/*  Traegt eine Initialisierung in eine const_list ein.         */
{
	...
	if (ISARRAY(f)) {
		...
		if (t->dsize) {
			...
			for (i = l2zm(0L);
					desi_follows
							|| ((zmeqto(t->size, l2zm(0L)) || !zmleq(t->size, i)
									|| ctok->type == LBRK) && ctok->type != RBRA);
					i = zmadd(i, l2zm(1L))) {
				if (!zmleq(i, 0)) {
					if (ctok->type == COMMA) {
						next_token();
				...
				if (!cl) {
					/* no designator */
					cl = insert_cl(last, i);
					cl->other = initialization(t->next, c99 ? noconst : 0,
							level + 1, 0, fstruct, cl->other);
...
#endif
		prev = 0;
		if (level == 0 && !bracket && !c99)
			error(157);
		for (i = l2zm(0L);
				desi_follows
						|| (!zmleq(t->exact->count, i) && ctok->type != RBRA);
				i = zmadd(i, l2zm(1L))) {
			if (!desi_follows && (*t->exact->sl)[zm2l(i)].identifier[0] == 0)
				continue; /* unnamed bitfield */
			if (!zmleq(i, 0)) {
				if (ctok->type == COMMA) {
					next_token();
					killsp();
			...
			if (!cl) {
				...
				cl->other = initialization((*t->exact->sl)[zm2l(i)].styp,
						c99 ? noconst : 0, level + 1, 0,
						!fstruct && !bracket && zmeqto(i, l2zm(0L)) ?
								t->exact : fstruct, cl->other);
				if (cl->other && zmeqto(cl->other->idx, l2zm(-2L))) {
					if (fstruct) {
						first = cl->other;
						first->other = 0;
				...
				cl->other = initialization((*t->exact->sl)[zm2l(i)].styp,
						c99 ? noconst : 0, level + 1, 1, fstruct, cl->other);
			}
			if (cl->next == first)
				first = cl;
			if (desi && !bracket)
		...
		while (desi_follows) {
			...
			if (!cl) {
				...
				cl->other = initialization((*t->exact->sl)[0].styp,
						c99 ? noconst : 0, level + 1, 0,
						!fstruct && !bracket ? t->exact : fstruct, cl->other);
				if (cl->other && zmeqto(cl->other->idx, l2zm(-2L))) {
					if (fstruct) {
						first = cl->other;
						first->other = 0;
		...
		if (noconst
				&& (ISSTRUCT(tree->ntyp->flags) || ISUNION(tree->ntyp->flags))
				&& fstruct == tree->ntyp->exact) {
			first = mymalloc(CLS);
			first->tree = tree;
