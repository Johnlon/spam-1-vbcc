    ...
    //  Gibt ein IC aus.
{
    ...
    if(p->code>=LABEL&&p->code<=BRA){
        if(p->code==LABEL)
            ...
            emit(f,"\t%s L%d",ename[p->code],p->typf);
            if(p->q1.flags){ emit(f,",");probj(f,&p->q1,0);}
        }
        if(p->code==LABEL&&(p->flags&LOOP_COND_TRUE)) emit(f," (while-loop)");
        if(p->code==BRA&&(p->flags&LOOP_COND_TRUE)) emit(f," (to-loop-test)");
    }else{
        emit(f,"\t%s ",ename[p->code]);
        if(p->typf&VOLATILE) emit(f,"volatile ");
        if(p->typf&CONST) emit(f,"const ");
        if(p->typf&UNSIGNED) emit(f,"unsigned ");
        if(p->typf){
            if(ISVECTOR(p->typf))
                emit(f,"%s%d ",typname[VECTYPE(p->typf)&NQ],VECDIM(p->typf));
            else
                emit(f,"%s ",typname[p->typf&NQ]);
        }
        probj(f,&p->q1,q1typ(p));
        if(p->q2.flags){emit(f,",");probj(f,&p->q2,q2typ(p));}
        if(p->z.flags){emit(f,"->");probj(f,&p->z,ztyp(p));}
        if(p->code==ASSIGN||p->code==PUSH||p->code==POP||p->code==CALL)
            emit(f," size=%ld",zm2l(p->q2.val.vmax));
        if((p->code==SAVEREGS||p->code==RESTOREREGS)&&p->q1.reg)
            emit(f," except %s",regnames[p->q1.reg]);
        if(p->code==CONVERT)
            emit(f," from %s%s",(p->typf2&UNSIGNED)?"unsigned ":"",typname[p->typf2&NQ]);
        if(p->code==LSHIFT||p->code==RSHIFT)
            emit(f," shift-type %s%s",(p->typf2&UNSIGNED)?"unsigned ":"",typname[p->typf2&NQ]);
        if(p->code==ADDI2P||p->code==SUBIFP||p->code==SUBPFP||p->code==ADDRESS)
            emit(f," ptype=%s%s",(p->typf2&UNSIGNED)?"unsigned ":"",typname[p->typf2&NQ]);
        if(p->code==ASSIGN||p->code==PUSH)
            if(p->typf2) emit(f," align=%d\n",p->typf2);
    }
    if(p->code==CALL){
        emit(f," =>");
        if(p->call_cnt==0)
            ...
            for(i=0;i<p->call_cnt;i++)
                emit(f," %s",p->call_list[i].v->identifier);
        }

    }
    if(p->flags&EFF_IC) emit(f," (eff_ic)");
    emit(f,"\n");
#if 0
    if(p->code==CALL){
        int i;
    ...
    /*  Ausserdem muss max(abs(max))<=max(unsigned max).  */
{
    ...
    if(zmleq(x,zm)&&!zmeqto(x,zm)){
        emit(f,"-");
        zm=zum2zm(t_max(MAXINT));
        // if x < -MAXINT
        if(zmleq(x,zmsub(l2zm(0L),zm))&&!zmeqto(x,zmsub(l2zm(0L),zm))){
            /* watch out, because -x might be >LONG_MAX */
            /*  aufpassen, da -x evtl. >LONG_MAX    */
            zum=t_max(MAXINT);
            x=zmadd(x,zm);
        }else 
...
/* Tries to flip an IC so that q2 and z don't occupy a common register. Returns zero if that is not possible. */
...
{
    int f=t&NQ; // type without qlifiers
    if(!p) ierror(0);
        ...
        if(p[-1]=='\n'){
            *emit_p=0;
            ...
            if(emit_l==emit_f){
                /* FIXME: error check */
                fputs(emit_buffer[emit_f],f);
                if(strlen(emit_buffer[emit_f])!=0 && strcmp("\n", emit_buffer[emit_f])!=0) {
                    printf("ASM: %s", emit_buffer[emit_f]);
                }
                fflush(stdout);
                emit_f++;
                if(emit_f>=EMIT_BUF_DEPTH) emit_f=0;
