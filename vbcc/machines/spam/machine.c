...
// JL char *g_attr_name[]={"__interrupt","__zpage","__nocpr",0};
char *g_attr_name[]={0};

#define INTERRUPT 1
#define ZPAGE 2         // TODO JL NA
#define NOCOMPRESS 4
...
#define isacc(x) (isreg(x)&&(p->x.reg==ra||p->x.reg==rax))  // SPAM REGA can be accumulator and REGB can be X
...
static int q1reg,q2reg,zreg; // Z = q1 OP q2

//static char *ccs[]={"eq","ne","lt","ge","le","gt",""};  // TODO JL EXTEND TO SPAM1 LIST
static char *ccs[]={"eq","ne","lt","gt",""};  // SPAM1 LIST    Overflow / Neg / Zero / Carry

static char *logicals[]={"ora","eor","and"}; // JL or with accumulator / eor with acc / and with acc
...
static struct obj *cam(int flags,int base,long offset)
{
    ...
    obj.am=&am;
    am.flags=flags; // TODO JL knobble?
    am.base=base; // TODO JL knobble?
    am.offset=offset; // TODO JL knobble?
    return &obj;
}
*/
            ...
            if(regok(r,t,1)){
                ...
                if(regs[r]){
                    ...
                    if(p->code==COMPARE||p->code==TEST)
                        ierror(0);
                    if(regs[ra]){
                        // STASH A IN Y BEFORE TRASHING A BELOW
                        //emit(f,"\ttay\n"); // JL WHY TX A to Y ??
                        // NOT NEEDED AS SPAM DOESNT TRASH THE ACC emit(f,"\tregc = rega\n"); // JL WHY TX A to Y ??

                        // not trashing Y ,  Y is REGC
                        //yval=NOVAL;
                    }
                    if(r1){
                        //emit(f,"\tlda\t%s\n",mregnames[r1]);
                        ...
                        emit(f,"\t; lda\t%s\n",mregnames[r1]);
                        emit(f,"\t; pha\n");
                        emit(f,"\tmarlo = [:stack_ptr]\n");
                        emit(f,"\tmarhi = [:stack_ptr+1]\n");
                        emit(f,"\tram   = :%s\n",mregnames[r1]);
                        emit(f,"\t; lda\t%s\n",mregnames[r2]);
                        emit(f,"\t; pha\n");
                        emit(f,"\tmarlo = marlo - 1 _S\n");
                        emit(f,"\tmarhi = marhi - 1 _C\n");
                        emit(f,"\tram   = :%s\n",mregnames[r2]);
                        emit(f,"\t[:stack_ptr]   = marlo - 1 _S\n");
                        emit(f,"\t[:stack_ptr+1] = marhi - 1 _C\n");
                    ...
                    if(regs[ra]) {
                        // RESTORE A
                        //emit(f,"\ttya\n");
                        // NOT NEEDED AS SPAM DOESNT TRASH THE ACC emit(f,"\trega = regc\n"); // JL WHY TX A to Y ??
                    }
                }
                if(r1){
                    regs[r1]|=flag;
...
static void reload_acc_opt(FILE *f,IC *p)
{
    ...
    if(pushedacc==-1){
        emit(f,"\t; pla\n");
        emit(f,"\tmarlo = [:stack_ptr] + 1 _S\n");
        emit(f,"\tmarhi = 0 A_PLUS_B_PLUS_C [:stack_ptr+1]\n");
        emit(f,"\tREGA = ram\n");
        emit(f,"\t[:stack_ptr]   = marlo\n");
        emit(f,"\t[:stack_ptr+1] = marhi\n");
    }
...
static void emit_lobyte(FILE *f,obj *o,int t)
{
    if(o->flags&KONST){
        if(o->flags&DREFOBJ){
            ...
            emit(f,"%d",zm2l(vmax)&255); // JL JUST REMOVED HASH - NOT SURE WHAT THIS IS ? MAYBE 8 bit immed if #, vs something else without the #?
        }
        ...
        //emit_obj(f,o,t);
        //emit(f,")");
        emit(f,">("); // assume it's an obj name?
        emit_obj(f,o,t);
        emit(f,")");
...
static void emit_lobyte_SPAM(FILE *f,obj *o,int t)
{
    if(o->flags&KONST){
        if(o->flags&DREFOBJ){
            eval_const(&o->val,o->dtyp);
...
static void emit_hibyte(FILE *f,obj *o,int t)
{
    if(o->flags&KONST){
        if(o->flags&DREFOBJ){
            ...
            eval_const(&o->val,t);
            if(ISFLOAT(t)) cnv_fp();
            //emit(f,"#%d",(zm2l(vmax)>>8)&255); // JL Just remove the #
...
static void emit_byte3(FILE *f,obj *o,int t)
{
    if(o->flags&KONST){
        if(o->flags&DREFOBJ){
            ...
            emit(f,"%d",(zm2l(vmax)>>16)&255); // DONT EMIT THE #
        }
    }else if((o->flags&(REG|DREFOBJ))==REG){
        emit(f,":%s+2",mregnames[o->reg]);
    }else if(o->flags&VARADR){
        //emit(f,"#%d",bank(o->v));
        emit(f,"%d",bank(o->v)); // JL NO #
    }else{
        if(!(o->flags&DREFOBJ))
...
static void emit_byte4(FILE *f,obj *o,int t)
{
    if(o->flags&KONST){
        if(o->flags&DREFOBJ){
            ...
            emit(f,"%d",(zm2l(vmax)>>24)&255); // NO HASH
        }
    }else if((o->flags&(REG|DREFOBJ))==REG){
...
static char* toSpamReg(char* name) {
    if (name[0] == 'a') return "A";
    if (name[0] == 'x') return "B";
    if (name[0] == 'y') return "C";
    if (name[0] == 'z') return "D";
    printf("CANNOT TRANSLATE REGISTER NAME '%s'\n", name);
    //ierror(0);
...
static void do_lobyte(FILE *f,char *s,obj *o,int type)
{
    ...
    if (strcmp("lda", s)==0) {
       // emit(f,"\t%s\t",s);
        emit(f,"\t; do_lobyte %s\n",s);
        emit(f,"\tREGA = ");
        emit_lobyte_SPAM(f,o,type);
    } else if (strcmp("sta", s)==0) {
        //emit(f,"\t%s\t",s);
        emit(f,"\t; do_lobyte %s\n",s);
        emit(f,"\t[");
        emit_lobyte(f,o,type);
        emit(f,"] = REGA");
    } else if (strcmp("adc", s)==0) {
        emit(f,"\t; do_lobyte NOT DONE %s\n",s);
        emit_lobyte(f,o,type);
        emit(f,"\t; do_lobyte END NOT DONE %s\n",s);
    } else {
        printf("CANNOT MAP do_lobyte FOR %s\n", s);
        //ierror(0);
    }
...
//    emit(f,"\t%s\t",s);
    ...
    if (strcmp("lda", s)==0) {
       // emit(f,"\t%s\t",s);
        ...
        emit(f,"\t; do_hibyte %s\n",s);
    } else if (strcmp("ldx", s)==0) {
        emit(f,"\t; do_hibyte %s\n",s);
    } else {
        printf("CANNOT MAP do_hibyte FOR %s\n", s);
        //ierror(0);
    }
...
static void do_byte3(FILE *f,char *s,obj *o,int type)
{
    ...
    if (strcmp("lda", s)==0) {
       // emit(f,"\t%s\t",s);
        ...
        printf("CANNOT MAP do_byte3 FOR %s\n", s);
        //ierror(0);
    }
...
static void do_byte4(FILE *f,char *s,obj *o,int type)
{
    ...
    if (strcmp("lda", s)==0) {
       // emit(f,"\t%s\t",s);
        ...
        printf("CANNOT MAP do_byte4 FOR %s\n", s);
        //ierror(0);
    }
...
{
    if((o->flags&REG)&&(o->reg==rx||o->reg==rax)) {
        emit(f,"\t; store_hibyte tax\n");
        emit(f,"\ttax\n");
    } else {
        emit(f,"\t; store_hibyte sta\n");
        do_hibyte(f,"sta",o,t);
    }
}
...
static void store_acc(FILE *f,obj *o,int type)
{
    ...
    if(!ISCHAR(type)){
        if(indirect(o)){
            /*TODO: save accu */
            emit(f,"\t; store_acc txa\n");
            emit(f,"\ttxa\n");
            store_hibyte(f,o,type);
        }else {
            emit(f,"\t; store_acc stx\n");
...
*/
    ...
    if(zbuf&&!NOBANKVARS){
        ...
        if(isreg(z)){
            r=0;
            if((p->q1.flags&(REG|DREFOBJ))==(REG|DREFOBJ)&&p->q1.reg==p->z.reg){
                r=get_reg(f,p,POINTER);
                cmp_get_acc(f,p,0);
            ...
            if((p->q2.flags&(REG|DREFOBJ))==(REG|DREFOBJ)&&p->q2.reg==p->z.reg){
                if(r==0){
                    r=get_reg(f,p,POINTER);
                    cmp_get_acc(f,p,0);
                    if(!reg_pair(p->z.reg,&rp)) ierror(0);
...
char* emitjl(FILE *f,const char *fmt,...)
{
  static char tmp[EMIT_BUF_LEN];
  char *p;
  va_list vl;
  if(!f||no_emit) return 0;
  va_start(vl,fmt);
  vsprintf(tmp,fmt,vl);
  p=tmp;
  while(*p){
...
emit(f,"@@V A|R");
            ...
            emit(f,"MARLO = [:%s]", mregnames[fp]);
            emit(f,"MARHI = [:%s+1]", mregnames[fp]);
...
/* JL junk
    if(!optsize||(v->tattr&NOCOMPRESS)) 
        emit(f,";vcprmin=10000\n");
    if(vlas) 
        emit(f,"___fo\tset\t%ld\n",(long)argsize);
*/
    ...
    /*  The main code-generation.                                           */
{
    ...
    // func uses variable length arrays
    /*
    FPVLA_REG - An additional register used in functions containing vlas. The backend can
    specify a register (usually framepointer) that can not be used in functions with
    vlas. Therefore, it is possible to use this register in other functions (for example,
    if local variables are usually addressed directly through the stackpointer).
    */
    if(vlas){
        fp=FPVLA_REG;
    ...
    for(pass=0;pass<2;pass++){
        ...
        /*FIXME*/  // JL NO INTERRUPT SUPPORTED
