/*
 * Backend for the SPAM-1 CPU
 * (c) 2022 by John Lonergan
 */

/*
JL

Stack and Frame pointer https://people.cs.rutgers.edu/~pxk/419/notes/frames.html
FP is set to SP on function entry and does not move. SP can move during function.

The link https://people.cs.rutgers.edu/~pxk/419/notes/frames.html uses a similar syntax to VBCC such as "movl $123, –8(sp)" (the page says rpb but that's the x86 sp)
It says "References to local variables will be negative offsets to the frame pointer" I assume because the FP was set at FN call and then the local vars were
laid down py pushing them onto stack and stack grows downwards towards 0 so relatively the var is a negative address relative to the FP.
...
Explanation: For "foo", a func that has no args calling "bar", a fn that takes two 4 byte (int) args.

- main func entry (no args passed)
  - push fp (ie of current fn) onto stack
  - set fp to current sp
- func call
  - push a 4 byte arg onto stack (sp is now a lower addr)
  - push a 4 byte arg onto stack (sp is now a lower addr)
  # at this point the two args sit in memory below the FP but above the SP
- subroutine entry
  - push fp (ie of current fn) onto stack
  - set fp to current sp
  # at this point the function arguments have an address higher in memory than the FP, ie positive
...
/* Name and copyright. */
char cg_copyright[]="vbcc code-generator for SPAM-1 v0.1 (c) 2022 by John Lonergan";
...
// JL #define ISTSHORT(t) (((t)&NQ)==CHAR || ((t)&NQ)==SHORT || ((t)&NQ)==INT || ((t)&NQ)==DPOINTER || ((t)&NQ)==PPOINTER)
#define ISTSHORT(t) (((t)&NQ)==CHAR || ((t)&NQ)==SHORT || ((t)&NQ)==INT || ((t)&NQ)==POINTER )
 ...
 *       `------------------------------------------------'
 */
// JL needs better explanation
  static long
_gc_real_offset(struct gc_state *gc, struct Var *v, long add_offset)
{
// JL offset relative to what?
  long off = zm2l(v->offset);

  if (off < 0) {
// JL less than 0 JL needs better explanation
// see my refs to SP/FP documentation
// local vars would have -v pos vs FP
  ...
  if (var)
  {
    ...
    if (isauto(var->storage_class)) {
      ...
      if (sp_offset <= 15) {
        ...
        int is_pair = 0; // removed this global - we don't support switching pairs like that as we're 8 bit we need something better than combining 16 bit reg's
        printf("\n============= JL REGPAIR\n");
        if (is_pair) {
          /* 32 bit */
...
//        if (!tiny) _gc_emit(gc, "\timm\t$(hi(%s+%d))\n", sym, ptr_const);
  ...
  if (ptr_reg)
  {
   ...
   int is_pair = 0; // JL removed this global - we don't support switching pairs like that as we're 8 bit we need something better than combining 16 bit reg's
    if (is_pair) {
      /* 32 bit */
      _gc_emit_mov(gc, R_A, rp.r1);
      _gc_emit(gc, "\t%s\tA, [X++]\n", opcode);
  ...
  if (!ptr_reg && !var)
  {
   ...
   int is_pair = 0; // JL removed this global - we don't support switching pairs like that as we're 8 bit we need something better than combining 16 bit reg's
    if (is_pair) {
      ...
      _gc_emit_mov(gc, R_A, rp.r1);
      printf("\n============= JL tiny???\n");
      int tiny=0; // removed global  fixme
      _gc_emit(gc, "\t%s\tA, [$%d]\n", opcode, tiny ? (ptr_const+0) : _gc_emit_imm(gc, ptr_const+0));
      _gc_emit_mov(gc, R_A, rp.r2); _gc_emit(gc, "\t%s\tA, [$%d]\n", opcode, tiny ? (ptr_const+1) : _gc_emit_imm(gc, ptr_const+1));
    } else {
      /* 16 bit access */
      _gc_emit_mov(gc, R_A, val_reg);
      printf("\n============= JL tiny???\n");
      int tiny=0; // removed global  fixme
      _gc_emit(gc, "\t%s\tA, [$%d]\n", opcode, tiny ? ptr_const : _gc_emit_imm(gc, ptr_const));
    }
  }
}
  ...
  */
  ...
  if (var)
  {
    ...
    if (isauto(var->storage_class)) {
      ...
      /* Make access using absolute immediate addressing */
   int is_pair = 0; // JL removed this global - we don't support switching pairs like that as we're 8 bit we need something better than combining 16 bit reg's
   int tiny=0; // JL disabled tiny
      if (is_pair) {
        /* 32 bit */
        if (!tiny) _gc_emit(gc, "\timm\t$(hi(%s+%d))\n", sym, ptr_const);
        _gc_emit(gc, "\t%s\tA, [$(lo(%s+%d))]\n", opcode, sym, ptr_const);
        //if (dtyp == PPOINTER) _gc_emit_nop(gc);
        if (dtyp == POINTER) _gc_emit_nop(gc);
        _gc_emit_mov(gc, rp.r1, R_A);
        if (!tiny) _gc_emit(gc, "\timm\t$(hi(%s+%d))\n", sym, ptr_const+1);
        _gc_emit(gc, "\t%s\tA, [$(lo(%s+%d))]\n", opcode, sym, ptr_const+1);
        //if (dtyp == PPOINTER) _gc_emit_nop(gc);
...
#define ULLONG	(UNSIGNED|LLONG)
  ...
  declare_builtin("__mulint32i",  LONG,  LONG,  R_R0P, LONG,  R_R2P, 1, 0);
...
// JL: This defines the register that will be used for the function return value 
...
// jl need explanation of how these are dangerous 
  int c=p->code;
...
// JL is it desirable to perform a given operation code directly on the shorter type t rather than requiring vbcc to promote value to int?
// jl return 1 to do the op on the narower type and 0 to allow the compiler to promote to a longer type as it desires 
...
/*
- p is list of IC's making up this fn
- v is function being generated
- offset is the space needed for local vars in bytes

This function has to take care that only scratchregisters are destroyed by this
function. The array regused contains information about the registers that
have been used by vbcc in this function. However if the code generator uses
additional registers it has to take care of them, too.
The regs[] and regused[] arrays may be overwritten by gen_code() as well
as parts of the list of ICs. However the list of ICs must still be a valid list of
ICs after gen_code() returns
...
void gen_var_head(FILE *f, struct Var *v)
{
  ...
  if (isstatic(v->storage_class) || isextern(v->storage_class))
  {
    /* Select section */
    ierror(0);
    //JL DISABLES AS DONT KNOW WHERE STR_PMEM IS section_type  = (attr && strstr(attr, STR_PMEM)) ? 4 : 0;
