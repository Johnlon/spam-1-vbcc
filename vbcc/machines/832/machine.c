...
//				settempobj(f,zreg,&p->z,0,0);
            } else if (sizetab[q1typ(p) & NQ] >=
                       sizetab[ztyp(p) & NQ]) {    // Reducing the size, must mask off excess bits...
                if (DBGMSG)
...
//						if(!isstackparam(&p->z) || (p->z.flags&DREFOBJ))
                    ...
                    if (!isreg(q1) || !isreg(z) ||
                        q1reg != zreg) // Do we just need to mask in place, or move the value first?
                    {
                        if (!isreg(z))
                            zreg = t1;
...
//				cleartempobj(f,t1);
        ...
        if (c == COMPARE) {
            ...
            if (!isreg(q1)) {
                ...
                emit_objtoreg(f, &p->q2, t, tmp);
            if ((!(q1typ(p) & UNSIGNED)) &&
                (!(q2typ(p) & UNSIGNED))) {    // If we have a mismatch of signedness we treat as unsigned.
...
int emit_peephole(void) {
    ...
    while (i != emit_l) {
        ...
        if (sscanf(emit_buffer[i], "\tmr\tr%d", &reg) == 1) {
            ...
            havemr = havemt = 0;
            if (reg == reg2 &&
                reg == 6) /* Only stack ops - others would be risky due to potential hardware registers. */
            {
                loadidx = i;
                haveload = 1;
