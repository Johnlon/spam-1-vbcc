  ...
  /*  Gibt Objekt auf Bildschirm aus                      */
{
  ...
  if (p->flags & REG) {
    if (p->reg > 8) {
      ...
      t &= NQ;
      if (t == CHAR && !(p->flags & DREFOBJ))
        emit(f, "%%%cl", regnames[p->reg][2]);
      else if (t == SHORT && !(p->flags & DREFOBJ))
        emit(f, "%%%s", regnames[p->reg] + 2);
      else
...
static char *mregname(int r, int t) {
  ...
  t &= NQ;
  if (t == CHAR)
    sprintf(s, "%%%cl", regnames[r][2]);
  else if (t == SHORT)
    sprintf(s, "%%%s", regnames[r] + 2);
  else
    sprintf(s, "%s", regnames[r]);
  return s;
}
...
oloop:
  unordered = 0;
  for (i = 0; i < 8; i++) {
    if (fst[i] > 0 && fst[i] != i + 9 && regs[fst[i]]) {
      unordered = 1;
      break;
    }
  }
...
static int get_reg(FILE *f, struct IC *p, int type) {
  ...
  for (i = 1; i <= 8; i++) {
    if (regs[i] < 2 && regok(i, type, 0) && (!(p->q1.flags & REG) || p->q1.reg != i) && (!(p->q2.flags & REG) || p->q2.reg != i) && (!(p->z.flags & REG) || p->z.reg != i) && (p->code != SETRETURN || p->z.reg != i) && (p->code != GETRETURN || p->q1.reg != i)) {
      if (p->code == PUSH || p->code == CALL) {
  ...
  /*  linkage etc.                                        */
{
  ...
  if (v->storage_class == STATIC) {
    ...
    if (v->clist && constflag && !(g_flags[3] & USEDFLAG) && section != CODE) {
      emit(f, codename);
      if (f) section = CODE;
    }
  ...
  for (; p; pr(f, p), p = p->next) {
    ...
    if (c == ASSIGN || c == PUSH) {
      ...
      if (!ISSCALAR(t) || !zmeqto(p->q2.val.vmax, sizetab[t & NQ]) || !zmleq(p->q2.val.vmax, l2zm(4L))) {
        ...
        if (regs[msi] || !regused[msi]) {
          m |= 2;
          if (!sil) sil = ++label;
          emit(f, "\tmovl\t%s,%s%d\n", regnames[msi], labprefix, sil);
        }
        if (regs[mdi] || !regused[mdi]) {
          m |= 4;
          if (!dil) dil = ++label;
    ...
    if (!isconst(q2) && (c == LSHIFT || c == RSHIFT)) {
      ...
      if (((p->q1.flags & REG) && p->q1.reg == cx) || ((p->z.flags & REG) && p->z.reg == cx) || (!compare_objects(&p->q1, &p->z) && !isreg(q1))) {
        fl = regs[cx];
        regs[cx] = 2; /* don't want cx */
        reg = get_reg(f, p, t);
        regs[cx] = fl;
