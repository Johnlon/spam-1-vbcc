...
static void ldq_offset(FILE *f, obj *o) {
    ...
    } else if ((o->flags & (VAR | REG | VARADR)) == VAR &&
               (o->v->storage_class == AUTO || o->v->storage_class == REGISTER)) {
        emit(f, "\tldz\t#%ld\n", real_offset(o));
    }
}
...
static void preload_obj(FILE *f, IC *p, obj *o) {
    ...
    if ((o->flags & (VAR | REG)) == VAR && (o->v->storage_class == AUTO || o->v->storage_class == REGISTER) &&
        (((of = real_offset(o)) + zm2l(szof(o->v->vtyp)) > 255) || longm65)) {
    ...
    if ((o->flags & (DREFOBJ | KONST)) == DREFOBJ && (!(o->flags & REG) || !isptr(o->reg)) &&
        (!(o->flags & VAR) || !(o->v->tattr & ZPAGE)) && !ISFPOINTER(o->dtyp)) {
        if (p->code == GETRETURN && (p->q1.reg == ra || p->q1.reg == rax)) {
            emit(f, "\tsta\t%s\n", mregnames[t1]);
            pa = 1;
        } else
...
static void preload(FILE *f, IC *p) {
    ...
    if (((p->q1.flags & DREFOBJ) && p->q1.am && (p->q1.am->flags == GPR_IND || p->q1.am->flags == ABS_IND) &&
         p->q1.am->idx == ra) ||
        ((p->q2.flags & DREFOBJ) && p->q2.am && (p->q2.am->flags == GPR_IND || p->q2.am->flags == ABS_IND) &&
         p->q2.am->idx == ra) ||
        ((p->z.flags & DREFOBJ) && p->z.am && (p->z.am->flags == GPR_IND || p->z.am->flags == ABS_IND) &&
         p->z.am->idx == ra)
            ) {
        if (pushedacc > 0)
            r = pushedacc;
        ...
        if ((p->q1.flags & DREFOBJ) && p->q1.am && (p->q1.am->flags == GPR_IND || p->q1.am->flags == ABS_IND) &&
            p->q1.am->idx == ra)
            p->q1.am->idx = r;
        if ((p->q2.flags & DREFOBJ) && p->q2.am && (p->q2.am->flags == GPR_IND || p->q2.am->flags == ABS_IND) &&
            p->q2.am->idx == ra)
            p->q2.am->idx = r;
        if ((p->z.flags & DREFOBJ) && p->z.am && (p->z.am->flags == GPR_IND || p->z.am->flags == ABS_IND) &&
            p->z.am->idx == ra)
            p->z.am->idx = r;
    }
...
static void peephole(struct IC *p) {
    ...
    for (; p; p = p->next) {
        ...
        /* Try const(reg) */
        if ((c == ADDI2P || c == SUBIFP) && ISPREG(z) && (p->q2.flags & (KONST | DREFOBJ)) == KONST &&
            !ISFPOINTER(p->typf2)) {
            int base;
            zmax of;
            struct obj *o;
                ...
                for (p2 = p->next; p2; p2 = p2->next) {
                    ...
                    if (c2 != CALL && (c2 < LABEL || c2 > BRA)/*&&c2!=ADDRESS*/) {
                        ...
                        if ((c2 == ASSIGN | c2 == PUSH) && (p2->typf & NQ) == CHAR && !zmeqto(p2->q2.val.vmax, Z1))
                            ...
                            mc = 0;
                        if (!o && (c2 == ADD || c2 == SUB || c2 == AND || c2 == OR || c2 == XOR) &&
                            /*(p2->typf&NQ)==CHAR&&*/!p2->q1.am && !p2->z.am &&
        ...
        /* Try reg,reg */
        if (c == ADDI2P && (p->typf & NU) == (UNSIGNED | CHAR) && !ISFPOINTER(p->typf2) &&
            isreg(q2)/*&&p->q2.reg!=ra*/&& ISPREG(z) && (ISPREG(q1) || p->q2.reg != p->z.reg)) {
            int base, idx, ind;
            struct obj *o;
            IC *free_idx, *dub = 0;
            r = p->z.reg;
...
static void function_top(FILE *f, struct Var *v, long offset) {
    ...
    if (libsave) {
        ...
        int last = 0;
        for (i = FIRST_GPR; i <= LAST_GPR; i++) {
            if ((regused[i] && (in_isr || !regscratch[i])) ||
                (in_isr && function_calls && regscratch[i] && !regsa[i])) {
                if (rscnt != rsavesize) {
                    if (ce02 && last == i - 1) {
                        emit(f, "\tphw\t%s\n", mregnames[last]);
                        last = 0;
...
int init_cg(void) {
    ...
    declare_builtin("__muluint32", UNSIGNED | LONG, UNSIGNED | LONG, FIRST_BIG + 1, UNSIGNED | LONG, FIRST_BIG + 2, 1,
                    0);
...
int cost_savings(struct IC *p, int r, struct obj *o) {
    ...
    if ((c == ADD || c == SUB) && o == &p->z && p->q1.flags == p->z.flags && p->q1.v == p->z.v &&
        (p->q2.flags & (KONST | DREFOBJ)) == KONST) {
        eval_const(&p->q2.val, q2typ(p));
        if (zmeqto(vmax, Z1) && zumeqto(vumax, ZU1))
            return co + 1;
    }
    ...
    if (ISLONG(t) && LONGM65(c)) {
        ...
        if (c == ADD) {
            if ((p->q2.flags & (KONST | DREFOBJ)) == KONST) {
                ...
                else if (zmeqto(vmax, l2zm(2L))) {
                    noop2 = 1;
                    emit(f, "\tinq\n\tinq\n");
                }
                else if (zmeqto(vmax, l2zm(3L))) {
                    noop2 = 1;
                    emit(f, "\tinq\n\tinq\n\tinq\n");
                }
                else if (zmeqto(vmax, l2zm(4L))) {
                    noop2 = 1;
                    emit(f, "\tinq\n\tinq\n\tinq\n\tinq\n");
                }
            }
    ...
    for (pass = 0; pass < 2; pass++) {
        ...
        for (p = pass == 0 ? p : mi; p; pr(f, p), p = p->next) {
            ...
            if (c == ADD || c == MULT || c == AND || c == XOR || c == OR ||
                (c == ADDI2P && ISSHORT(t) && !ISFPOINTER(p->typf2))) {
            ...
            if (c == COMPARE &&
                ((p->q1.flags & (KONST | DREFOBJ)) == KONST || ISRIDX(q2) || (isacc(q2) && !ISRIDX(q1)))) {
                obj tmp;
                IC *b;
                tmp = p->q1;
                p->q1 = p->q2;
            ...
            if ((c == ADD || c == SUB || c == ADDI2P || c == SUBIFP) && compare_objects(&p->q1, &p->z) &&
                (!indirect(&p->q1) || (isreg(q1) && ISIDX(p->q1.reg))) && isconst(q2) && !isacc(z)) {
                long l;
                eval_const(&p->q2.val, q2typ(p));
                l = zm2l(vmax);
                if (c == ADDI2P/*&&(t2&NQ)==POINTER*/) {
                    c = ADD;
                    t = UNSIGNED | INT;
                }
                if (c == SUBIFP/*&&(t2&NQ)==POINTER*/) {
                    c = SUB;
                    t = UNSIGNED | INT;
                }
	  ...
	  stack_valid=0;
            ...
            if (c == ADDRESS) {
                ...
                if (isacc(z)) {
                    ...
                    if (o < 256) {
                        ++label;
                        emit(f, "\tldx\t%s\n\tbcc\t%s%d\n\tinx\n%s%d:\n", mregnames[fp2], labprefix, label, labprefix,
                             label);
                        continue;
                    }
                    if (o < 512) {
                        ++label;
                        emit(f, "\tldx\t%s\n\tinx\n\tbcc\t%s%d\n\tinx\n%s%d:\n", mregnames[fp2], labprefix, label,
                             labprefix, label);
                        continue;
                    }
                }
            ...
            if (c == COMPARE || c == TEST) {
                ...
                if ((t & NU) == (UNSIGNED | CHAR) && (bc == BLE || bc == BGT) &&
                    (p->q2.flags & (DREFOBJ | KONST)) == KONST) {
                    eval_const(&p->q2.val, t);
                ...
                if (((t & NQ) == SHORT || (t & NQ) == INT || (t & NQ) == LONG) && (bc == BNE || bc == BEQ) &&
                    isconst(q2) && !isacc(q1)) {
                    eval_const(&p->q2.val, t);
                ...
                if (ISCHAR(t)) {
                    char *s = 0;
                    if (isreg(q1) && ISIDX(p->q1.reg) && !indirect(&p->q2) &&
                        (bc == BEQ || bc == BNE || (t & UNSIGNED))) {
                        static char buf[4] = "cpr";
                        s = buf;
                        s[2] = mregnames[p->q1.reg][0];
                    } else {
                        if (pacc = cmp_get_acc(f, p, branch))
                            bout = ++label;
                        load_acc(f, &p->q1, t);
            ...
            if (c == LSHIFT || c == RSHIFT) {
                long l = -1, m;
                int loop = 0, r = 0, r2 = 0, r3 = 0, outl = 0, same = 0;
                if (isconst(q2)) {
                    eval_const(&p->q2.val, q2typ(p));
                    l = zm2l(vmax);
                    loop = 0;
            ...
            if ((c >= OR && c <= AND) || (c >= LSHIFT && c <= MOD) || c == ADDI2P || c == SUBIFP) {
                ...
                if (!isacc(z)) {
                    ...
                    if ((c == ADD || c == SUB || c == AND || c == XOR || c == OR || c == ADDI2P || c == SUBIFP) &&
                        isacc(q1) && scratch(p->next, ra, 1));
                    else {
                ...
                if (ISFLOAT(t)) {
                    ...
                    if (cnst && c == AND && (l & 0xff) == 0) emit(f, "\tlda\t#0\n");
                        ...
                        load_lobyte(f, &p->q1, t);
                        if (!(cnst && ((c == AND && (l & 0xff) == 0xff) || (c == OR && (l & 0xff) == 0) ||
                                       (c == XOR && (l & 0xff) == 0))))
                            do_lobyte(f, s, &p->q2, t);
                    }
                    store_lobyte(f, &p->z, t);
                    if (cnst && c == AND && (l & 0xff00) == 0) emit(f, "\tlda\t#0\n");
                        ...
                        load_hibyte(f, &p->q1, t);
                        if (!(cnst && ((c == AND && (l & 0xff00) == 0xff00) || (c == OR && (l & 0xff00) == 0) ||
                                       (c == XOR && (l & 0xff00) == 0))))
                    ...
                    if (cnst && c == AND && (l & 0xff0000) == 0) emit(f, "\tlda\t#0\n");
                        ...
                        do_byte3(f, "lda", &p->q1, t);
                        if (!(cnst && ((c == AND && (l & 0xff0000) == 0xff0000) || (c == OR && (l & 0xff0000) == 0) ||
                                       (c == XOR && (l & 0xff0000) == 0))))
                            do_byte3(f, s, &p->q2, t);
                    }
                    do_byte3(f, "sta", &p->z, t);
                    if (cnst && c == AND && (l & 0xff000000) == 0) emit(f, "\tlda\t#0\n");
                        ...
                        do_byte4(f, "lda", &p->q1, t);
                        if (!(cnst &&
                              ((c == AND && (l & 0xff000000) == 0xff000000) || (c == OR && (l & 0xff000000) == 0) ||
                               (c == XOR && (l & 0xff000000) == 0))))
                            do_byte4(f, s, &p->q2, t);
                    }
                    do_byte4(f, "sta", &p->z, t);
                    continue;
                    ...
                    if (l > 0 && ((c == AND && (l & 0xFF) == 0) || (c == OR && (l & 0xFF) == 0xFF))) {
                        dello = 1;
                               ...
                               ((c == AND && (l & 0xFF) == 0xFF) || ((c == OR || c == XOR) && (l & 0xFF) == 0))) {
                        if (!compare_objects(&p->q1, &p->z)) {
                            load_lobyte(f, &p->q1, t);
                            store_lobyte(f, &p->z, t);
                        }
                    ...
                    if (l > 0 && ((c == AND && (l & 0xFF00) == 0) || (c == OR && (l & 0xFF00) == 0xFF00))) {
                        ...
                        if (isacc(z)) {
                            ...
                            store_hibyte(f, &p->z, t);
                               ...
                               ((c == AND && (l & 0xFF00) == 0xFF00) || ((c == OR || c == XOR) && (l & 0xFF00) == 0))) {
                        if (isacc(z) && !indirect(&p->q1)) {
                            do_hibyte(f, "ldx", &p->q1, t);
                        } else {
                            if (!compare_objects(&p->q1, &p->z)) {
...
int shortcut(int code, int typ) {
    if (code == COMPARE || code == MULT || code == ADD || code == SUB || code == AND || code == OR || code == XOR ||
        code == LSHIFT || code == RSHIFT || code == MINUS || code == KOMPLEMENT || code == NEGATION)
        return 1;
...
int emit_peephole(void) {
    ...
    if ((j = sscanf(asmline[0], " %6s %999s", op1, buf1)) >= 1) {
      ...
      printf("\t\t%s %s\n",op1,buf1);*/
        if (!strcmp(op1, "lda")) {
            if (buf1[0] == '#') {
                if (!rm && !strcmp(buf1, ca)) {
                    remove_asm();
                    return rm = 1;
                }
                if (!rm && !strcmp(buf1, cx)) {
                    strcpy(asmline[0], "\ttxa\n");
                    return rm = 1;
                }
                if (!rm && !strcmp(buf1, cy)) {
                    strcpy(asmline[0], "\ttya\n");
                    return rm = 1;
                }
                if (!rm && !strcmp(buf1, cz)) {
                    strcpy(asmline[0], "\ttza\n");
                    return rm = 1;
                }
                strcpy(ca, buf1);
            } else ca[0] = 0;
...
char *use_libcall(int c, int t, int t2) {
    ...
    if (c == COMPARE) {
        if ((t & NQ) == LLONG || (ISFLOAT(t) && !ieee)) {
            sprintf(fname, "__cmp%s%s%ld", (t & UNSIGNED) ? "u" : "s", ISFLOAT(t) ? "flt" : "int",
                    zm2l(sizetab[t & NQ]) * 8);
            ret = fname;
        }
    } else {
        ...
        if (c == CONVERT) {
            ...
            if (ISFLOAT(t)) {
                sprintf(fname, "__%cint%ldtoflt%d", (t2 & UNSIGNED) ? 'u' : 's', zm2l(sizetab[t2 & NQ]) * 8,
                        (t == FLOAT) ? 32 : 64);
                ret = fname;
            }
            if (ISFLOAT(t2)) {
                sprintf(fname, "__flt%dto%cint%ld", ((t2 & NU) == FLOAT) ? 32 : 64, (t & UNSIGNED) ? 'u' : 's',
                        zm2l(sizetab[t & NQ]) * 8);
                ret = fname;
            }
        }
        ...
        if ((t & NQ) == SHORT || (t & NQ) == INT || (t & NQ) == LONG || (t & NQ) == LLONG || (!ieee && ISFLOAT(t))) {
            if ((c >= LSHIFT && c <= MOD) || (c >= OR && c <= AND) || c == KOMPLEMENT || c == MINUS) {
                ...
                if (t == (UNSIGNED | LLONG) && (c == MULT || c == DIV || c == MOD || c == RSHIFT)) {
                    ...
                    sprintf(fname, "__%s%s%s%ld", ename[c], (t & UNSIGNED) ? "u" : "", ISFLOAT(t) ? "flt" : "int",
                            zm2l(sizetab[t & NQ]) * 8);
                    ret = fname;
                }
            }
...
/*  Gibt atyps aus.                                     */
{
    ...
    if (t == (UNSIGNED | LONG) || t == FPOINTER || t == HPOINTER) {
        fprintf(f, "UL");
        vumax = zul2zum(p->vulong);
        printzum(f, vumax);
    }
