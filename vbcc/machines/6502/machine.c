...
#define chk_coll(x)                                                     \
    do {                                                                \
        if ((x) == r || (x) == r1 || (x) == r2) return 0;               \
        if (reg_pair((x), &rp) && (rp.r1 == r || rp.r2 == r)) return 0; \
    } while (0)
	  ...
	  stack_valid=0;
            ...
            if (c == ADDRESS) {
                ...
                if (isacc(z)) {
                    ...
                    if (o < 256) {
                        ++label;
                        emit(f, "\tldx\t%s\n\tbcc\t%s%d\n\tinx\n%s%d:\n", mregnames[fp2], labprefix, label, labprefix, label);
                        continue;
                    }
                    if (o < 512) {
                        ++label;
            ...
            if ((c >= OR && c <= AND) || (c >= LSHIFT && c <= MOD) || c == ADDI2P || c == SUBIFP) {
                ...
                if (ISFLOAT(t)) {
                    ...
                    if (cnst && c == AND && (l & 0xff) == 0)
                        emit(f, "\tlda\t#0\n");
                    else if (cnst && c == OR && (l & 0xff) == 0xff)
                        emit(f, "\tlda\t#255\n");
                    else {
                        load_lobyte(f, &p->q1, t);
                    ...
                    store_lobyte(f, &p->z, t);
                    if (cnst && c == AND && (l & 0xff00) == 0)
                        emit(f, "\tlda\t#0\n");
                    else if (cnst && c == OR && (l & 0xff00) == 0xff00)
                        emit(f, "\tlda\t#255\n");
                    else {
                        load_hibyte(f, &p->q1, t);
                    ...
                    store_hibyte(f, &p->z, t);
                    if (cnst && c == AND && (l & 0xff0000) == 0)
                        emit(f, "\tlda\t#0\n");
                    else if (cnst && c == OR && (l & 0xff0000) == 0xff0000)
                        emit(f, "\tlda\t#255\n");
                    else {
                        do_byte3(f, "lda", &p->q1, t);
                    ...
                    do_byte3(f, "sta", &p->z, t);
                    if (cnst && c == AND && (l & 0xff000000) == 0)
                        emit(f, "\tlda\t#0\n");
                    else if (cnst && c == OR && (l & 0xff000000) == 0xff000000)
                        emit(f, "\tlda\t#255\n");
                    else {
                        do_byte4(f, "lda", &p->q1, t);
