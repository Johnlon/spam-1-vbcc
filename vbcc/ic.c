...
{
    IC *new = new_IC();
    if (!ISPOINTER(z->ntyp->flags)) {
        error(39);
        return;
    }
    if (!ISPOINTER(q->ntyp->flags)) {
        error(39);
        return;
    }
...
/*  fuegt ein IC ein                                            */
{
    ...
    if (last_ic) {
        if (code == ASSIGN) {
            if ((last_ic->z.flags & (REG | SCRATCH | DREFOBJ)) == (REG | SCRATCH) &&
                (new->q1.flags == last_ic->z.flags) && last_ic->z.reg == new->q1.reg/*&&last_ic->code!=CALL*/) {
                if (USEQ2ASZ || !(last_ic->q2.flags & REG) || !(new->z.flags & REG) || last_ic->q2.reg != new->z.reg) {
                    if (USEQ2ASZ || !(last_ic->q2.flags & VAR) || !(new->z.flags & VAR) || last_ic->q2.v != new->z.v) {
                        ...
                        if ((new->q1.flags & (SCRATCH | REG)) == (SCRATCH | REG) &&
                            (new->q1.reg != new->z.reg || !(new->z.flags & REG)))
                            free_reg(new->q1.reg);
                        free(new);
                        return;
                    }
            ...
            if ((last_ic->z.flags & (VAR | SCRATCH | DREFOBJ)) == (VAR | SCRATCH) &&
                (new->q1.flags == last_ic->z.flags) && last_ic->z.v == new->q1.v/*&&last_ic->code!=CALL*/) {
                if (USEQ2ASZ || !(last_ic->q2.flags & REG) || !(new->z.flags & REG) || last_ic->q2.reg != new->z.reg) {
                    if (USEQ2ASZ || !(last_ic->q2.flags & VAR) || !(new->z.flags & VAR) || last_ic->q2.v != new->z.v) {
        ...
        if (last_ic->code == BRA) {
            ...
            if (last_ic->prev && code == LABEL) {
                ...
                if (last_ic->prev->code >= BEQ && last_ic->prev->code <= BGT && new->typf == last_ic->prev->typf) {
                    ...
                    if (DEBUG & 1)
                        printf("%s l%d;%s l%d; substituted\n", ename[last_ic->prev->code], last_ic->prev->typf,
                               ename[last_ic->code], last_ic->typf);
    ...
    if (p->flags == STRING) {
        ...
        if (merge_strings) {
            for (v = first_var[0]; v; v = v->next) {
                if (v->vtyp->flags == ARRAY && (v->vtyp->next->flags & STRINGCONST) &&
                    zmleq(p->ntyp->size, v->vtyp->size)) {
                    zmax i;
                    const_list *cl1 = p->cl, *cl2 = v->clist;
                    for (i = Z0;
                         !zmleq(p->ntyp->size, i) && cl1 && cl2; cl1 = cl1->next, cl2 = cl2->next, i = zmadd(i, Z1)) {
...
#endif
        new->code = COMPARE;
        tl = p->left->ntyp->flags & NU;
        tr = p->right->ntyp->flags & NU;
        if (p->right->flags == CEXPR && ISINT(tr) && ISINT(tl) && zm2l(sizetab[tl & NQ]) <= zm2l(sizetab[tr & NQ]) &&
            ((tl & NQ) >= INT || shortcut(COMPARE, tl))) {
            int negativ;
            eval_constn(p->right);
        ...
        if (p->left->flags == ADDRESS && p->left->left->flags == IDENTIFIER) {
            ...
            if (v->fi && v->fi->first_ic && !cross_module && (c_flags_val[0].l & 4096)) {
                ...
                for (vp = v->fi->vars; vp; vp = vp->next) {
                    ...
                    if ((zmeqto(vp->offset, l2zm(0L)) || vp->reg) && *vp->identifier &&
                        (vp->storage_class == AUTO || vp->storage_class == REGISTER))
                        arg_cnt++;
                }
                ...
                while (al) {
                    /*FIXME: zmeqto hier ok? siehe cross_module_inline */
                    while (vp && (!*vp->identifier || (!zmeqto(vp->offset, l2zm(0L)) && !vp->reg) ||
                                  (vp->storage_class != REGISTER && vp->storage_class != AUTO)))
                        vp = vp->next;
                    if (!vp) {
                        error(39);
                        break;
            ...
            if (v->storage_class == EXTERN) {
                if ((optflags & 2) && (!strcmp(v->identifier, "strlen") || !strcmp(v->identifier, "__asm_strlen")) &&
                    p->alist && p->alist->arg) {
                    np n = p->alist->arg;
                    if (n->flags == ADDRESSA && n->left->flags == STRING && zmeqto(n->left->val.vmax, l2zm(0L))) {
                        const_list *cl;
                        zumax len = ul2zum(0UL);
                        cl = n->left->cl;
                        while (cl) {
                ...
                if (inline_memcpy_sz > 0 && (optflags & 2)) {
                    if ((!strcmp(v->identifier, "strcpy") || !strcmp(v->identifier, "__asm_strcpy")) && p->alist &&
                        p->alist->next && p->alist->next->arg) {
            ...
            if (ffreturn(p->ntyp)) {
                ...
                if (ISSTRUCT(t) || ISUNION(t)) {
                    ...
                    if (optflags & 2) {
                        ...
                        int r;
                        r = new->q1.reg;
                        if (regs[r] || !regok(r, p->ntyp->flags, 0) ||
                            (reg_pair(r, &rp) && (regs[rp.r1] || regs[rp.r2]))) {
                            r = 0;
                        } else {
                            for (l2 = rl; l2; l2 = l2->next) {
                                if (l2->v && abs(l2->reg) == r) {
                                    r = 0;
                                    break;
                                }
                            }
...
/*  nach double konvertiert, falls kein Prototype da ist.                   */
{
    ...
    if (!sd) ierror(0);
    if (n < sd->count) {
        ft = clone_typ((*sd->sl)[n].styp);
        sz = szof(ft);
        t = ft->flags;
        reg = (*sd->sl)[n].reg;
    } else {
        ft = clone_typ(al->arg->ntyp);
        sz = szof(ft);
        t = ft->flags;
    }
    if (ISINT(t)) {
    ...
    if (reg) {
        ...
        if (optflags & 2) {
            ...
            *rl = nrl;
            ...
            if (reg_pair(reg, &rp) && (arg->flags & (REG | SCRATCH)) == (REG | SCRATCH) &&
                (arg->reg == rp.r1 || arg->reg == rp.r2)) {
            ...
            if (!regs[reg] && (!reg_pair(reg, &rp) || (!regs[rp.r1] && !regs[rp.r2])) && !nocode) {
                ...
                if (reg_pair(reg, &rp)) {
                    regs[rp.r1] = 1 | RNOSAVE;
                    regused[rp.r1]++;
                    regs[rp.r2] = 1 | RNOSAVE;
                    regused[rp.r2]++;
                }
	      ...
	      al->pushic=new;
            ...
            if ((new->q1.flags & REG)) {
                ...
                for (p = *rl; p; p = p->next) {
                    int r;
                    if (p->v && (r = abs(p->reg))) {
                        if (new->q1.reg == r || (reg_pair(new->q1.reg, &rp) && (rp.r1 == r || rp.r2 == r))) {
                            new->q1.v = p->v;
                            new->q1.val.vmax = l2zm(0L);
                            break;
                        }
                        /*FIXME: andersrum bei LITTLEENDIAN?? */
                        if (reg_pair(r, &rp) && rp.r1 == new->q1.reg) {
                            new->q1.v = p->v;
                            new->q1.val.vmax = l2zm(0L);
                            break;
                        }
                        if (reg_pair(r, &rp) && rp.r2 == new->q1.reg) {
                            new->q1.v = p->v;
                            new->q1.val.vmax = regsize[rp.r1];
                            break;
                        }
...
/* removed */
    ...
    if (!volatile_convert && ((o & (NU & ~UNSIGNED)) == (f & (NU & ~UNSIGNED)) ||
                              (!(mc = must_convert(o, f, const_expr)) && (const_expr || !(optflags & 2))))) {
        p->ntyp->flags = f;
        if (!ISPOINTER(f) && !ISARRAY(f)) {
            freetyp(p->ntyp->next);
            p->ntyp->next = 0;
        }
        return;
...
/*  Fordert Register fuer Typ f an.                     */
{
    ...
    if (nocode) return (1);
    for (i = 1; i <= MAXR; i++) {
        if (!regs[i] && reg_prio[i] > prio && regok(i, f, mode) &&
            (!reg_pair(i, &rp) || (!regs[rp.r1] && !regs[rp.r2]))) {
            r = i;
            prio = reg_prio[i];
        }
    }
...
/*  liefert 0, wenn dest nicht gefunden                             */
{
    ...
    if (ISPOINTER(p->left->ntyp->flags) && ISPOINTER(p->right->ntyp->flags)) {
        ...
        if (!zmleq(szof(p->left->ntyp->next), l2zm(1L))) {
            ...
            eval_const(&gval, MAXINT);
            if (ln = get_pof2(vumax)) {
                /* Division immer ohne Rest, daher shift m�glich */
    ...
    if ((p->flags == ADD || p->flags == SUB) && (ISPOINTER(p->left->ntyp->flags) || ISPOINTER(p->right->ntyp->flags))) {
        ...
        if (new->typf & UNSIGNED) {
            if ((new->typf & NQ) < (MINADDUI2P & NQ)) {
                convert(p->right, MINADDUI2P);
                new->typf = MINADDUI2P;
            }
        } else {
            if ((new->typf & NQ) < MINADDI2P) {
                convert(p->right, MINADDI2P);
                new->typf = MINADDI2P;
            }
        }
        /*  groessere Typen als MAXADDI2P erst in diesen wandeln */
        if (new->typf & UNSIGNED) {
            if ((new->typf & NQ) > (MAXADDI2P & NQ)) {
                convert(p->right, MAXADDUI2P);
                new->typf = MAXADDUI2P;
            }
        } else {
            if ((new->typf & NQ) > MAXADDI2P) {
                convert(p->right, MAXADDI2P);
                new->typf = MAXADDI2P;
            }
        }
        ...
        new->q2 = p->right->o;
        if (!dest && (p->left->o.flags & (SCRATCH | REG)) == (SCRATCH | REG) &&
            regok(new->q1.reg, POINTER_TYPE(p->left->ntyp->next), p->left->ntyp->next->flags)) {
            new->z = p->left->o;
            new->z.flags &= (~DREFOBJ);
        } else {
